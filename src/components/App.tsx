...
export const ShinyNavbar: React.FC<propsItem> = (props) => {
  ...
  const handlerOnClick = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>, index: number, item: NavbarItem) => {
    setSelected(index)
    if (item.url) {
        ...
        <div className='shiny-navbar__box'>
          ...
          {items.map((item, index) => {
            ...
            const selectedBorder = `shiny-borders_${
              index === 0 ? 'left' : index + 1 === items.length ? 'right' : 'mid'
            }`
            return (
              <button
                key={index}
                onClick={(e) => handlerOnClick(e, index, item)}
                className={`shiny-navbar__element${isSelected ? ' shiny-selected' : ''}`}
              >
                {isSelected && (
                  <>
                    <motion.div
